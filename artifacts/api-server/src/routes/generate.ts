...
// Global helper for fetch-based web search (uses raw config)
async function getRawAIConfig() {
  const result = await db.execute(sql`SELECT * FROM ai_configurations WHERE is_active = true LIMIT 1`);
  const dbConfig = result.rows[0];
  return {
    baseUrl: dbConfig?.base_url || process.env.AI_INTEGRATIONS_OPENAI_BASE_URL || "https://api.openai.com/v1",
    apiKey: dbConfig?.api_key || process.env.AI_INTEGRATIONS_OPENAI_API_KEY || "",
  };
}
...
// Run one web search query and return the raw text output
async function webSearch(query: string, timeoutMs = 50000): Promise<string> {
  ...
  try {
    ...
    const config = await getRawAIConfig();
    const res = await fetch(`${config.baseUrl}/responses`, {
      ...
      signal: (AbortSignal as any).timeout?.(timeoutMs) || null,
    }) as any;
    ...
    for (const o of (data.output ?? [])) {
      if (o.type === "message") {
        text = o.content?.find(c => c.type === "output_text")?.text ?? "";
        if (text.trim()) break;
      }
    }
